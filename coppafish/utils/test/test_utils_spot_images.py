...
def test_get_spot_images():
    ...
    output_2d = spot_images.get_spot_images(image_2d, spot_yxz_2d, shape_2d)
    output_3d = spot_images.get_spot_images(image_3d, spot_yxz_3d, shape_3d)
    ...
    output_2d = spot_images.get_spot_images(image_2d, spot_yxz_2d, np.array(shape_2d))
    output_3d = spot_images.get_spot_images(image_3d, spot_yxz_3d, np.array(shape_3d))
    assert np.allclose(output_2d, expected_output_2d), '2d output was not expected'
    assert np.allclose(output_3d, expected_output_3d), '3d output was not expected'
    # Test the out of bounds mechanic
    assert np.all(np.isnan(spot_images.get_spot_images(
...
def test_get_average_spot_image():
    ...
    output_2d_mean   = spot_images.get_average_spot_image(spot_images_2d)
    output_3d_mean   = spot_images.get_average_spot_image(spot_images_3d)
    output_2d_median = spot_images.get_average_spot_image(spot_images_2d, av_type='median')
    output_3d_median = spot_images.get_average_spot_image(spot_images_3d, av_type='median')
    ...
    assert np.allclose(spot_images.get_average_spot_image(spot_images_2d,   'mean'), np.mean  (fill_values[2:])), \
        'Unexpected 2d output with nan values'
    assert np.allclose(spot_images.get_average_spot_image(spot_images_3d,   'mean'), np.mean  (fill_values[2:])), \
        'Unexpected 3d output with nan values'
    assert np.allclose(spot_images.get_average_spot_image(spot_images_2d, 'median'), np.median(fill_values[2:])), \
        'Unexpected 2d output with nan values'
    assert np.allclose(spot_images.get_average_spot_image(spot_images_3d, 'median'), np.median(fill_values[2:])), \
        'Unexpected 3d output with nan values'
