...
def extract_and_filter(config: dict, nbp_file: NotebookPage,
    ...
    # save scale values in case need to re-run
...
def par_extract_and_filter(config: dict, nbp_file: NotebookPage,
    ...
    if config['deconvolve']:
        ...
        wiener_filter = extract.get_wiener_filter(psf, pad_im_shape, config['wiener_constant'])
        nbp_debug.psf = psf
        nbp_debug.psf_intensity_thresh = None  # TODO problem here
        nbp_debug.psf_tiles_used = psf_tiles_used
    else:
        nbp_debug.psf = None
        nbp_debug.psf_intensity_thresh = None
    ...
    for r in tqdm(use_rounds, desc='Rounds'):
        ...
        # convolve_2d each image
        kwargs = dict()
        kwargs['nbp_basic'] = nbp_basic
        kwargs['nbp_file'] = nbp_file
        ...
        for t in nbp_basic.use_tiles:
            for c_id, c in enumerate(use_channels):
                ...
                nbp_debug.clip_extract_scale[t, r, c] = results[t][3][c_id]

                if r != nbp_basic.anchor_round:
                    nbp.hist_counts[:, r, c] += results[t][1][c_id]
    ...
    for c in use_channels:
        ...
        if file_exists:
            ...
            if r == nbp_basic.anchor_round and c == nbp_basic.dapi_channel:
                ...
                # if r != nbp_basic.anchor_round:
                #     hist_counts[:, r, c] += hist_counts_trc
        else:
            ...
            if r == nbp_basic.anchor_round and c == nbp_basic.dapi_channel:
                ...
                auto_thresh_c, hist_counts_trc_c, n_clip_pixels_c, clip_extract_scale_c = \
                    extract.get_extract_info(im[:, good_columns], config['auto_thresh_multiplier'],
                                             hist_bin_edges, max_tiff_pixel_value, scale, z_info)

                # Deal with pixels outside uint16 range when saving
                if n_clip_pixels_c > config['n_clip_warn']:
                    warnings.warn(f"\nTile {t}, round {r}, channel {c} has "
                                  f"{n_clip_pixels_c} pixels\n"
                                  f"that will be clipped when converting to uint16.")
                if n_clip_pixels_c > config['n_clip_error']:

                    message = f"\nTile {t}, round{r}, channel {c} has more than {config['n_clip_error']} pixels " \
                              f"clipped in conversion to uint16"
        ...
        n_clip_pixels.append(n_clip_pixels_c)
        clip_extract_scale.append(clip_extract_scale_c)

    return [auto_thresh, hist_counts_trc, n_clip_pixels, clip_extract_scale]
