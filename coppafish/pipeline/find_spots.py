...
def find_spots(
    ...
    if image_t is not None:
        assert len(nbp_basic.use_tiles) == 1, \
    ...
    with tqdm(total=np.sum(uncompleted), postfix={'tile': t, 'round': r, 'channel': c}, 
              desc=f"Detecting spots on filtered {nbp_extract.file_type} images") as pbar:
        ...
        for t, r, c in np.argwhere(uncompleted):
            ...
            no_negative_neighbour = fs.check_neighbour_intensity(image_trc, local_yxz,
            ...
            if r == nbp_basic.anchor_round:
                isolated_spots = fs.get_isolated(image_trc.astype(np.int32) - nbp_basic.tile_pixel_value_shift,
