...
def get_tilepos(xy_pos, tile_sz):
    """
    tilepos_yx['nd2'][i, 0] is y index of tile with fov index i in nd2 file.
    tilepos_yx['nd2'][i, 1] is x index of tile with fov index i in nd2 file.
    tilepos_yx['tiff'][i, 0] is y index of tile with tile directory (tiff files) index i.
    tilepos_yx['tiff'][i, 1] is x index of tile with tile directory (tiff files) index i.
    ...
    :return:
        tilepos_yx: dictionary.
        ['nd2']: integer numpy array [nTiles x 2]
        ['tiff']: integer numpy array [nTiles x 2]
    """
    tilepos_yx = {'nd2': np.zeros_like(xy_pos, dtype=int)}
    if np.shape(xy_pos)[0] != 1:
        ...
        tilepos_yx['nd2'][:, 0] = np.flip(np.arange(ny).repeat(nx))
        tilepos_yx['nd2'][:, 1] = np.tile(np.concatenate((np.flip(np.arange(nx)), np.arange(nx))),
                                          np.ceil(ny / 2).astype(int))[:np.shape(xy_pos)[0]]
    t_tiff_index = get_tile_file_indices(tilepos_yx['nd2'])
    tilepos_yx['tiff'] = tilepos_yx['nd2'][np.argsort(t_tiff_index)]
    return tilepos_yx


def get_tile_file_indices(tilepos_yx):
    """
    Tile with nd2 fov index i is at yx position tilepos_yx[i, :]
    ...
    """
    ny, nx = tuple(np.max(tilepos_yx, 0) + 1)
    tile_file_index = np.ravel_multi_index(np.array([tilepos_yx[:, 0], tilepos_yx[:, 1]]), (ny, nx))
