...
def extract_and_filter(config, log_file, log_basic):
    ...
    if config['r_dapi'] is None:
        log_extract['r_dapi'] = extract.get_pixel_length(log_extract['r_dapi_auto_microns'], log_basic['pixel_size']['xy'])
    ...
    if config['scale'] is None:
        ...
        log_extract['scale_tile'], log_extract['scale_channel'], log_extract['scale_z'], log_extract['scale'] = \
            extract.get_scale(im_file, log_extract['scale_tile'], log_extract['scale_channel'], log_extract['scale_z'],
                              log_basic['tilepos_yx'], log_basic['use_tiles'], log_basic['use_channels'],
    ...
    for r in use_rounds:
        ...
        if r == log_basic['anchor_round']:
            ...
            if log_extract['scale_anchor'] is None:
                log_extract['scale_anchor_tile'], _, log_extract['scale_anchor_z'], log_extract['scale_anchor'] = \
                    extract.get_scale(im_file, log_extract['scale_anchor_tile'], log_basic['anchor_channel'],
                                      log_extract['scale_anchor_z'], log_basic['tilepos_yx'],
                                      log_basic['use_tiles'], use_channels, log_basic['use_z'],
        ...
        for t in log_basic['use_tiles']:
            for c in range(log_basic['n_channels']):
                if c in use_channels:
                    ...
                    # if not including channel, just set to all zeros (2D, in 3D just don't save)
