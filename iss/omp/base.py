...
def fit_coefs(bled_codes: np.ndarray, pixel_colors: np.ndarray, weight: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    for i in range(max_genes):
        ...
        with tqdm(total=n_continue, disable=no_verbose) as pbar:
            ...
            if i == 0:
                ...
                for g in range(n_genes):
                    use = added_genes[:, i] == g
                    residual_pixel_colors[:, use], all_coefs[use, g:g+1] = fit_coefs(bled_codes[:, g:g+1], pixel_colors[:, use])
                ...
                for s in np.where(continue_pixels)[0]:
                    ...
                    if weight_coef_fit:
                        residual_pixel_colors[:, s:s+1], all_coefs[s, added_genes[s, :i+1]] = \
                            fit_coefs(bled_codes[:, added_genes[s, :i+1]], pixel_colors[:, s:s+1], weight[:, s:s+1])
                    else:
