...
def find_spots(config, nbp_file, nbp_basic, auto_thresh):
    ...
    # have to save spot_yx and spot_isolated as table to stop pickle issues associated with numpy object arrays.
    # columns of spot_details are: tile, channel, round, isolated, y, x, z
    spot_details = np.empty((0, 6+int(nbp_basic['3d'])), dtype=int)
    nbp['spot_no'] = np.zeros_like((nbp_basic['n_tiles'], nbp_basic['n_channels'],
        ...
        for r in use_rounds:
            ...
            for t in nbp_basic['use_tiles']:
                for c in use_channels:
                    ...
                    spot_yx, spot_intensity = fs.detect_spots(image, auto_thresh[t, c, r],
                                                              nbp_params['radius_xy'], nbp_params['radius_z'])
                    no_negative_neighbour = fs.check_neighbour_intensity(image, spot_yx, thresh=0)
                    spot_yx = spot_yx[no_negative_neighbour]
                    ...
                    if r == nbp_basic['ref_round']:
                        ...
                        spot_isolated = fs.get_isolated(image, spot_yx, isolation_thresh,
                        ...
                        spot_yx = spot_yx[descend_intensity_arg[:nbp_params['max_spots']]]
                        # don't care if these spots isolated so say they are not
                        spot_isolated = np.zeros(spot_yx.shape[0], dtype=bool)
                    spot_details_tcr = np.zeros((spot_yx.shape[0], spot_details.shape[1]), dtype=int)
                    spot_details_tcr[:, :3] = [t, c, r]
                    spot_details_tcr[:, 3] = spot_isolated
                    spot_details_tcr[:, 4:] = spot_yx
                    spot_details = np.append(spot_details, spot_details_tcr, axis=0)
                    nbp['spot_no'][t, c, r] = spot_yx.shape[0]
                    pbar.update(1)
