...
def find_spots(config, nbp_file, nbp_basic, auto_thresh):
    ...
    nbp['spot_no'] = np.zeros((nbp_basic['n_tiles'], nbp_basic['n_channels'],
        ...
        for r in use_rounds:
            ...
            for t in nbp_basic['use_tiles']:
                for c in use_channels:
                    ...
                    spot_yxz, spot_intensity = fs.detect_spots(image, auto_thresh[t, c, r],
                    ...
                    spot_details_tcr = np.zeros((spot_yxz.shape[0], spot_details.shape[1]), dtype=int)
                    spot_details_tcr[:, :3] = [t, c, r]
                    spot_details_tcr[:, 3] = spot_isolated
                    spot_details_tcr[:, 4:4+spot_yxz.shape[1]] = spot_yxz  # if 2d pipeline, z coordinate set to 0.
                    spot_details = np.append(spot_details, spot_details_tcr, axis=0)
                    nbp['spot_no'][t, c, r] = spot_yxz.shape[0]
                    pbar.update(1)
