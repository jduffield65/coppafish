...
def iterate(yxz_base, yxz_target, transforms_initial, n_iter, dist_thresh, matches_thresh, scale_dev_thresh,
    ...
    :param transforms_initial: numpy float array [dim+1 x dim x n_tiles x n_rounds x n_channels]
        transforms_initial[:, :, t, r, c] is the starting affine transform for tile t
        from the reference image to round r, channel c.
        transforms_initial[:dim, :dim] is probably going to be the identity matrix.
        transforms_initial[dim, :] is the shift which needs to be pre-computed somehow to get a good result.
    ...
    transforms_outlier: numpy float array [dim+1 x dim x n_tiles x n_rounds x n_channels]
        transforms[:, :, t, r, c] is the final affine transform found for tile t, round r, channel c
